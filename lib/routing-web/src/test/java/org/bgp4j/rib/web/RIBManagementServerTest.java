...
//		Assert.assertNull(client.routes("foo", RIBSide.Local, AddressFamily.IPv4, null));
	...
	public void testListOnePeerLocalIPv4UnicastRoutingBase() throws Exception {
		...
		UUID ribID = prib.routingBase(RIBSide.Local, AddressFamilyKey.IPV4_UNICAST_FORWARDING).getRibID();
		
		Assert.assertNotNull(ribID);
		...
		Assert.assertEquals(ribID, entry.getRibID());
	...
	public void testListOnePeerLocalIPv4UnicastRemoteIPv4UnicastRoutingBase() throws Exception {
		...
		prib.allocateRoutingInformationBase(RIBSide.Local, AddressFamilyKey.IPV4_UNICAST_FORWARDING);
		UUID localUUID = prib.routingBase(RIBSide.Local, AddressFamilyKey.IPV4_UNICAST_FORWARDING).getRibID();

		prib.allocateRoutingInformationBase(RIBSide.Remote, AddressFamilyKey.IPV4_UNICAST_FORWARDING);
		UUID remoteUUID = prib.routingBase(RIBSide.Remote, AddressFamilyKey.IPV4_UNICAST_FORWARDING).getRibID();

		Assert.assertFalse(localUUID.equals(remoteUUID));
		...
		Set<RouteInformationBaseDTO> it = ribCollection.getEntries();
		
		Assert.assertEquals(2, it.size());
		
		Assert.assertTrue(it.contains(new RouteInformationBaseDTO("foo", localUUID, AddressFamilyKey.IPV4_UNICAST_FORWARDING, RIBSide.Local)));
		Assert.assertTrue(it.contains(new RouteInformationBaseDTO("foo", remoteUUID, AddressFamilyKey.IPV4_UNICAST_FORWARDING, RIBSide.Remote)));
	...
	@Test
	public void testListOnePeerLocalIPv4UnicastRoutingBaseOneRouteByRibID() throws Exception {
		NetworkLayerReachabilityInformation nlri1 = new NetworkLayerReachabilityInformation(24, 
				new byte[] { (byte)0xc0, (byte)0xa8, (byte)0x01 });
		PeerRoutingInformationBase prib = pribManager.peerRoutingInformationBase("foo");
		Set<PathAttribute> pathAttributes = new TreeSet<PathAttribute>();
		...
		RouteCollection routeCollection = client.routes(rib.getRibID());
